...
export default function CreateProduct({}) {
  ...
  const nameRegex = /^[a-zA-Z0-9\s]+$/; // regex untuk nama produk
  const categoryRegex = /^[a-zA-Z\s]+$/; // regex untuk kategori produk
  const freshnessRegex = /^(new|second)$/i; // regex untuk kesegaran produk
  const priceRegex = /^\d+(\.\d{1,2})?$/; // regex untuk harga produk
  ...
  async function addProduct() {
    ...
    if (
      ...
      !categoryRegex.test(input.category) ||
      !freshnessRegex.test(input.freshness) ||
